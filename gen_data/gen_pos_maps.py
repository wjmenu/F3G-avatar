...
def interpolate_lbs(pts, vertices, faces, vertex_lbs):
    ...
    # print(dists.mean())
    if isinstance(vertex_lbs, np.ndarray):
        vertex_lbs_t = torch.from_numpy(vertex_lbs).to(torch.float32).cuda()
    else:
        vertex_lbs_t = vertex_lbs.to(torch.float32).cuda()
    lbs = barycentric_interpolate(
        vert_attris = vertex_lbs_t[None],
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if os.path.exists(data_dir + '/smplxpp_template.ply'):
            print('# Loading template from %s' % (data_dir + '/smplxpp_template.ply'))
            template_path = data_dir + '/smplxpp_template.ply'
            using_weight_smplxpp = True
        elif os.path.exists(config.PROJ_DIR + '/mesh/template/tpose/template.obj'):
            print('# Loading template from %s' % (config.PROJ_DIR + '/mesh/template/tpose/template.obj'))
            template_path = config.PROJ_DIR + '/mesh/template/tpose/template.obj'
            using_weight_smplxpp = True
        elif os.path.exists(data_dir + '/template.ply'):
            print('# Loading template from %s' % (data_dir + '/template.ply'))
            template_path = data_dir + '/template.ply'
            using_template = True
        else:
            template_path = None
            using_template = False
    ...
    elif using_weight_smplxpp:
        template_lbs_candidates = [
            os.path.join(data_dir, 'template_weights.npy'),
            os.path.join(config.PROJ_DIR, 'mesh/template/weights/template_weights.npy'),
        ]
        smplxpp_lbs_path = next((p for p in template_lbs_candidates if os.path.exists(p)), None)
        assert smplxpp_lbs_path is not None, (
            f"Expected template LBS weights at one of: {template_lbs_candidates}"
        )
        smplxpp_lbs = np.load(smplxpp_lbs_path).astype(np.float32)
        assert smplxpp_lbs.shape[0] == cano_smpl_v.shape[0], \
            f"SMPLX++ LBS vertex count {smplxpp_lbs.shape[0]} != template vertices {cano_smpl_v.shape[0]}"
        pts_lbs_np = interpolate_lbs(cano_pts, cano_smpl_v, smpl_faces, smplxpp_lbs)
        pts_lbs = torch.from_numpy(pts_lbs_np).cuda()
    else:
        pts_lbs_np = interpolate_lbs(cano_pts, cano_smpl_v, smpl_faces, smpl_model.lbs_weights)
        pts_lbs = torch.from_numpy(pts_lbs_np).cuda()
    ...
    # For debug/inspection: first 3 frames as PNG, and up to 100 random frames as PLY
    first_three = set(frame_list[:3]) if len(frame_list) > 0 else set()
    num_samples = min(100, len(frame_list))
    sampled_pose_idxs = set()
    if num_samples > 0:
        sampled_pose_idxs = set(np.random.choice(frame_list, size=num_samples, replace=False))
    ...
    for pose_idx in tqdm.tqdm(frame_list, desc = 'Generating positional maps...'):
        ...
        # Save first three frames as PNGs
        if pose_idx in first_three:
            png_path = data_dir + '/smpl_pos_map/%08d.png' % pose_idx
            vis = live_pos_map.copy()
            vmin = vis.min()
            vmax = vis.max()
            if np.isfinite(vmin) and np.isfinite(vmax) and vmax > vmin:
                vis = (vis - vmin) / (vmax - vmin)
            vis = np.clip(vis * 255.0, 0, 255).astype(np.uint8)
            cv.imwrite(png_path, vis)

        # Save up to 100 randomly chosen frames as PLY point clouds
        if pose_idx in sampled_pose_idxs:
            ply_path = data_dir + '/smpl_pos_map/posmap_%08d.ply' % pose_idx
            save_pos_map(live_pos_map, ply_path)
